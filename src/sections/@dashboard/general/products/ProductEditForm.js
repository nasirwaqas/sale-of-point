...
import { GET_PRODUCT_BY_ID, GET_CATEGORIES_BY_BRANCH } from '../../../../graphQL/queries';
...
export default function ProductEditForm() {
  ...
  const defaultValues = useMemo(
    () => ({
      skuCode: productData?.getProductById?.skuCode || '',
      code: productData?.getProductById?.code || '',
      name: productData?.getProductById?.name || '',
      category: productData?.getProductById?.category || '',
      company: productData?.getProductById?.company || '',
      formula: productData?.getProductById?.formula || '',
      purchasePrice: productData?.getProductById?.purchasePrice || '',
      salePrice: productData?.getProductById?.salePrice || '',
      wholesalePrice: productData?.getProductById?.wholesalePrice || '',
      saleQuantities: productData?.getProductById?.saleQuantities || '',
      discountPercentage: productData?.getProductById?.discountPercentage || '',
      discountRs: productData?.getProductById?.discountRs || '',
      minimumStock: productData?.getProductById?.minimumStock || '',
      boxPurchasePrice: productData?.getProductById?.boxPurchasePrice || '',
      boxSalePrice: productData?.getProductById?.boxSalePrice || '',
      boxSize: productData?.getProductById?.boxSize || '',
      saleMargin: productData?.getProductById?.saleMargin || '',
      description: productData?.getProductById?.description || '',
      location: productData?.getProductById?.location || '',
      inStock: productData?.getProductById?.inStock || '',
      addStock: productData?.getProductById?.addStock || '',
      image: productData?.getProductById?.image || null,
      branchId: productData?.getProductById?.branchId || '60d0fe4f5311236168a109ca', // Add branchId to default values
    }),
    [productData]
  );
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await editProduct({ variables: { id, input } });
  ...
  if (productError) return <Typography>Error loading product data</Typography>;
